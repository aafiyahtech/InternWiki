...
function getDistinctValues(field) {
    ...
    Promise.all([getDistinctValues("Industry"), getDistinctValues("Category")])
    .then(([industries, categories]) => {
    ...
    // Populate the category dropdown with the retrieved values
    const categoryDropdown = document.getElementById("category");
    categoryDropdown.innerHTML = "<option value=''>(Select an category)</option>";
    for (const category of categories) {
        industryDropdown.innerHTML += `<option value='${category}'>${category}</option>`;
      }
    });
